<x-app-layout>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            {{-- AÑADE EL COMPONENTE CAROUSEL AQUÍ --}}
            @isset($popularItems) {{-- Asegúrate de que $popularItems se pase a la vista --}}
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
                    ...
                    <div class="space-y-4">
                        @forelse ($activities as $activity)
                            <div class="flex gap-4 bg-[var(--bg-secondary)] p-4 rounded-lg border-l-4 border-[var(--border-color)] shadow-md"> {{-- Añadido shadow-md --}}
                                ...
                                <div class="w-full">
                                    {{-- Incluimos las vistas parciales de actividad que ya teníamos --}}
                        ...
                        @empty
                            <div class="bg-[var(--bg-secondary)] p-6 rounded-lg text-center shadow-lg"> {{-- Añadido shadow-lg --}}
                ...
                <aside class="space-y-6">
                    <div class="bg-[var(--bg-secondary)] p-6 rounded-lg text-center shadow-lg border border-[var(--border-color)]">
                        <img class="h-20 w-20 rounded-full mx-auto mb-2 border-2 border-[var(--text-primary)] object-cover" {{-- Añadido border-2 y object-cover --}}
                            src="{{ Auth::user()->avatar_path ? asset('storage/' . Auth::user()->avatar_path) : asset('images/default-avatar.png') }}" 
                            alt="Avatar de {{ Auth::user()->name }}">
                        <h4 class="font-bold text-lg text-[var(--text-primary)] mb-2">{{ Auth::user()->name }}</h4> {{-- Añadido color y mb-2 --}}
                        <a href="{{ route('profile.edit') }}" class="text-sm text-[var(--text-secondary)] hover:underline">Ver Perfil Completo</a> {{-- Añadido color --}}
                    </div>
                    <div class="bg-[var(--bg-secondary)] p-6 rounded-lg shadow-lg border border-[var(--border-color)]">
                        <h4 class="font-bold text-[var(--text-primary)] mb-4">Mi Progreso</h4> {{-- Añadido color --}}
                        <ul class="space-y-2 text-sm text-[var(--text-secondary)]"> {{-- Añadido color --}}
                    ...
                    <div class="bg-[var(--bg-secondary)] p-6 rounded-lg shadow-lg border border-[var(--border-color)]">
                        <h4 class="font-bold text-[var(--text-primary)] mb-4">Tendencias de la Semana</h4> {{-- Añadido color --}}
                        <ul class="space-y-3">
                            @forelse ($trendingItems as $item)
                                <li class="flex items-center gap-3">
                                    <img src="{{ $item->cover_image_url ?? asset('images/default-game-cover.png') }}" 
                                            alt="{{ $item->title }}" 
                                            class="w-10 h-14 object-cover rounded flex-shrink-0"> {{-- Añadido flex-shrink-0 --}}
                                    <div class="text-sm text-[var(--text-secondary)]">{{ $item->title }} ({{ $item->additions_count }} adiciones)</div> {{-- Añadido color --}}
                                </li>
                            @empty
